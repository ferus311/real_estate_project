...
def write_to_parquet(houses_json):
    ...
    # Tải file tạm lên HDFS
    with open(local_temp_file, "rb") as temp_file:
        hdfs_client.write(batch_name, temp_file, overwrite=True)

    # Xóa file tạm
    os.remove(local_temp_file)

    print(f"Saved batch to: {batch_name}")
...
def crawl_data_multithreaded(start_page, end_page, max_threads=5):
...
if __name__ == "__main__":
    ...
    END_PAGE = 500
    MAX_THREADS = 5
    crawl_data_multithreaded(START_PAGE, END_PAGE, MAX_THREADS)
    batch_upload_to_hdfs()  # Ensure remaining data is uploaded
