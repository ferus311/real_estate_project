"""
Trích xuất dữ liệu thô từ Batdongsan từ HDFS
"""
...
from common.schema.batdongsan_schema import get_batdongsan_raw_schema
...
def extract_batdongsan_data(
    ...
    """
    logger = SparkJobLogger("extract_batdongsan_data")
    ...
    # Tạo ID xử lý
    processing_id = generate_processing_id("batdongsan_extract")
    ...
    try:
        ...
        augmented_df = (
            ...
            .withColumn("data_source", lit("batdongsan"))  # Add data source identifier
        )
        ...
        output_path = os.path.join(
            bronze_path, f"batdongsan_{input_date.replace('-', '')}.parquet"
        )
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Extract Batdongsan Data")
...
if __name__ == "__main__":
    ...
    spark = create_spark_session("Extract Batdongsan Data")

    try:
        # Trích xuất dữ liệu
        extract_batdongsan_data(spark, args.date, args.property_type)
